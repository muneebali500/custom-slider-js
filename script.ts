...
const customersList = document.getElementById(`customers`) as HTMLElement;
const customerEl = document.getElementById(`customer`) as HTMLElement;
const leftArrowBtn = document.getElementById(`left-btn`) as HTMLButtonElement;
const rightArrowBtn = document.getElementById(`right-btn`) as HTMLButtonElement;
const customers = document.querySelectorAll(
  `.customer`
...
let prevElementIndex = 0;
let circleArr: NodeListOf<HTMLLIElement>;
...
/////////////////// FUNCTIONS //////////////////////////
function calculateSlideAmount(): number {
  const customerElWidth = customerEl.clientWidth;
  const customerElGap = Number(
    getComputedStyle(customersList).gap.replace(/\D+/g, "")
  );
...
function createCircles(): void {
  ...
  circleArr = document.querySelectorAll(`.circle`) as NodeListOf<HTMLLIElement>;
  circleArr[0].style.backgroundColor = `#fff`;
...
function handleSlide(direction: string): void {
  if (direction === `left`) {
    customersList.scrollLeft -= calculateSlideAmount();
    prevElementIndex -= prevElementIndex > 0 ? 1 : 0;
    setCircleBgColor(prevElementIndex);
...
function setCircleBgColor(circleIndex: number): void {
  circleArr.forEach((el, index) => {
    index === circleIndex
      ? (el.style.backgroundColor = `#fff`)
      : (el.style.backgroundColor = `transparent`);
